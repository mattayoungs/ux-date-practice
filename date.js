...
function getDayOfTheWeek(date) {
  ...
  let givenDay = date.getDay();
  if (week.indexOf("Sunday") === givenDay) {
    day = week[0];
  } else if (week.indexOf("Monday") === givenDay) {
    day = week[1];
  } else if (week.indexOf("Tuesday") === givenDay) {
    day = week[2];
  } else if (week.indexOf("Wednesday") === givenDay) {
    day = week[3];
  } else if (week.indexOf("Thursday") === givenDay) {
    day = week[4];
  } else if (week.indexOf("Friday") === givenDay) {
    day = week[5];
  } else if (week.indexOf("Saturday") === givenDay) {
    day = week[6];
  }
  return day;
}
...
function getDaysAgoString(date) {
  ...
  let today = new Date();
  today = today.getTime();
  givenDay = date.getTime();
  let numOfDays = Math.floor((today - givenDay) / aDay);
  let daysAgo = "";
  if (numOfDays === 0) {
    daysAgo = "Today";
  } else if (numOfDays === 1) {
